...
class ContainerManager:
    ...
    async def get_container_ip(self, container_id: str) -> str:
        ...
        if returncode != 0:
            raise ServiceException(SandboxErrorCode.DOCKER_RUNTIME_FAILED,f"docker inspect failed: {stderr}")

        container_ip = next((line.strip() for line in stdout.splitlines() if line.strip()), "")
        if not container_ip:
            raise ServiceException(SandboxErrorCode.DOCKER_RUNTIME_FAILED,f"docker inspect did not return container ip: {container_id}")
        return container_ip
