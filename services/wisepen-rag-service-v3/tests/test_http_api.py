...
from rag_v3.application.reading import DocumentReadError, ReadPageItem, ReadSectionItem
...
def api_client(monkeypatch):
    ...
    nacos = importlib.import_module("rag_v3.core.config.nacos")
...
"""
    ...
    router_module = importlib.import_module("rag_v3.api.router")
    container_module = importlib.import_module("rag_v3.container")
    reading_module = importlib.import_module("rag_v3.api.endpoints.reading")
    retrieval_module = importlib.import_module("rag_v3.api.endpoints.retrieval")
...
def test_query_routes_return_transport_shapes_and_context_scope(api_client) -> None:
    ...
    assert parent["matched_chunk_ids"] == ["chunk-1"]
    assert "source_spans" not in parent
    assert hybrid.json()["data"]["hits"][0]["node_ids"] == ["node-1"]
    ...
    assert neighborhood.json()["data"]["items"][0]["outline"] == "- Title [C] (5 chars)"
