...
from chat.application.chat_context_assembler import ChatContextAssembler, PreloadedSkillInstruction
...
from chat.application.skill_exposure import merge_skill_meta, normalize_skill_id, resolve_requested_ids
from chat.application.tools.skill_tools.utils.skill_matcher import SkillMatcher
from chat.application.tools.skill_tools.utils.builtin_skills import read_builtin_skill_asset
...
def _build_note_ai_diff_preloaded_skill(
    skill_id: str,
    available_skills: list,
) -> PreloadedSkillInstruction | None:
    try:
        content = read_builtin_skill_asset(skill_id, "/SKILL.md").decode("utf-8")
    except Exception as e:
        error("builtin note AI-Diff skill preload failed.", skill_id=skill_id, exc=e)
        return None

    skill_meta = next((skill for skill in available_skills if skill.skill_id == skill_id), None)
    return PreloadedSkillInstruction(
        skill_id=skill_id,
        name=skill_meta.name if skill_meta else "WisePen Note AI-Diff",
        content=content,
    )
...
class ChatTurnCoordinator:
    ...
    async def handle_chat(
        ...
        if note_context is not None:
            ...
            if note_context.client_content_signature:
                tool_context["active_note_client_content_signature"] = note_context.client_content_signature

        # 构建Skill视图
        # 返回本轮可展示给 LLM 的 Skill metadata，由 LLM 判断是否加载
        available_skills = []
        note_ai_diff_skill_id = normalize_skill_id(settings.NOTE_AI_DIFF_SKILL_ID)
        if tool_and_skill_policy.enable_use_tool and tool_and_skill_policy.enable_use_skill:
            ...
            if note_context is not None:
                force_enabled_skill_ids.add(note_ai_diff_skill_id)
        ...
        if note_context is not None and note_ai_diff_skill_id in available_skill_ids:
            note_ai_diff_skill = _build_note_ai_diff_preloaded_skill(
                note_ai_diff_skill_id,
                available_skills,
            )
            if note_ai_diff_skill is not None:
                preloaded_skill_instructions.append(note_ai_diff_skill)
        ...
        if note_context is not None and (
            note_ai_diff_skill_id in available_skill_ids
        ...
        messages_for_llm = self._context_assembler.assemble_prompt(
            ...
            preloaded_skill_instructions=preloaded_skill_instructions or None, # 已由系统注入的常驻技能
