...
class ToolRegistry:
    ...
    def derive(
        ...
        *,
        ...
        visible_tools: Dict[str, BaseTool] = {}
        ...
        for name, tool in tools.items():
            ...
            if not _allowed_by_user_filters(
                name,
            ...
            if tool.exposure == ToolExposure.BOOTSTRAP:
                visible_tools[name] = tool
            else:
                deferred_tools[name] = tool
...
def _allowed_by_user_filters(
    name: str,
    *,
    allow_tool_name_set: Optional[Set[str]],
    deny_tool_name_set: Optional[Set[str]],
) -> bool:
    if allow_tool_name_set is not None:
        return name in allow_tool_name_set
    if deny_tool_name_set is not None:
        return name not in deny_tool_name_set
    return True
