    ...
    def __init__(self, file_storage_client: FileStorageClient, session_repo: SessionRepository) -> None:
        ...
        self._definition = ToolDefinition(
            llm_spec=ToolLLMSpec(
                ...
                description=(
                    "Extract and read the text content of a PDF attachment. "
                    "Use this for .pdf files attached to the conversation."
                ),
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ToolExecutionError(reason="pdf_parser_unavailable", detail_reason="PDF parser (pdfplumber) is not installed.")
        except Exception as e:
            raise ToolExecutionError(reason="pdf_parse_failed", detail_reason=f"Failed to parse PDF: {e}") from e
