...
class RankingTokenizer:
    ...
    config: RankingTokenizerConfig = field(default_factory=RankingTokenizerConfig)
    stopwords: frozenset[str] = DEFAULT_STOPWORDS
    domain_lexicon: DomainLexicon = field(default_factory=DomainLexicon)
    _jieba_tokenizer: jieba.Tokenizer = field(init=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        tokenizer = jieba.Tokenizer()
        self.domain_lexicon.apply_to_jieba(tokenizer)
        object.__setattr__(self, "_jieba_tokenizer", tokenizer)
