...
class PdfConverter:
        ...
        *,
        ...
        try:
            ...
            raise RemoteParserTimeoutError(
                "PDF parsing request timed out."
            ) from exc
        except httpx.HTTPError as exc:
            raise RemoteParserError(
                f"PDF parsing request failed: {exc}."
            ) from exc
    ...
    async def _write_zip_response(
        ...
        *,
        ...
        if written == 0 or (
            "zip" not in content_type and bytes(signature) != b"PK"
        ):
            raise RemoteParserError(
                "PDF parser did not return a ZIP result."
            )


async def _response_preview(response: httpx.Response) -> str:
    preview = bytearray()

    async for chunk in response.aiter_bytes():
        preview.extend(chunk[: _ERROR_PREVIEW_BYTES - len(preview)])
