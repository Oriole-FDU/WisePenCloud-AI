...
from chat.application.tools.core import ToolRegistry, ToolSelectionMode, ToolSourceSpec
...
async def list_user_tools(
    ...
    return R.success(data=ListUserToolsResponse(
        tools=[
            build_tool_response(tool, configs.get(name))
...
async def get_user_tool_config(
    ...
    return R.success(data=build_tool_response(tool, config))
...
async def update_user_tool_config(
    ...
    return R.success(data=build_tool_response(tool, entity))
