...
`formal_pr` 是目标代码基线；原 RAG 工作树只用于确认 Kafka、父子 chunk、ES/Qdrant、ACL、证据物化和现有图增强的真实实现。
...
- child 是 ES/Qdrant 检索单元，parent 是回答上下文；Mongo 保存两者和 locator。
- ES/Qdrant、ranking、answerability、parent evidence materialization 和本地 ACL predicate 可以复用。
...
- 实体类型由 schema 和描述约束，角色说明属于当前 mention/关系，不污染 canonical anchor。
- 每个 mention 和 relation 都绑定输入片段的真实 ID 与原文 span。
- 共享 canonical anchor 用于发现跨 chunk、跨文档候选。
- 每跳先排序种子，并维护 visited set 和候选预算。

SAG 的强制 Event 中心模型不适合通用学习资料；`name.lower()` 的 exact 去重不足以解决同义词和同名异义；引用为空时回退到全部
items 会伪造证据。WisePen MVP 改用现有关系文档中的 `ConceptMention -> Concept`，引用无效时丢弃 candidate；`SemanticAnchor`
只是这一类跨文档锚点的方案层统称。
...
- `locate` 返回稳定 anchor/section/source ID；歧义 anchor 返回候选及证据。
...
| 初始定位     | 复用 ES/Qdrant 和 `RankingPipeline`                                               |
| 跨文档桥     | `AnchorMention -> SemanticAnchor` 与有证据的 typed relation                         |
| LLM 抽取   | `neo4j-graphrag` 的 `LLMEntityRelationExtractor`，固定 schema + structured output  |
| 概念解析     | exact alias -> 候选召回 -> evidence 判定；歧义时才调用 LLM                                  |
| 图存储/检索   | Neo4j，一至两跳固定 Cypher pattern                                                    |
| 原文回读     | Mongo SourceRef + `ToolContentStore`                                           |
| ACL      | Java Kafka -> 本地 ES/Qdrant/Neo4j projection                                    |
| 更新       | Resource version staged/applied；按 changed section/chunk 重算                     |

没有选择任何项目的完整 GraphRAG 路线。目标是把标题树、语义锚点和现有 RAG 组合成可验证的导航链路。
