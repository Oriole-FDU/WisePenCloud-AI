...
- 一个 chunk 可以包含多个完整的相邻短 Section，但必须保留每个 Section 的独立 span。
- 一个超长 Section 可以对应多个 chunk；Section 本身仍可通过自己的原文范围完整读取。
...
- 一个 page chunk 可以包含多个 Section，保存每个 Section 在本页的独立 span。
...
2. 相邻短 Section 属于同一父节点且合并后不超过 soft limit 时，可装入同一 chunk，但不合并 Section 身份。
3. 超长 Section 在内部按完整 block 装箱，允许最后一组达到字符硬上限。
4. 单个 block 超过硬上限时使用通用 Markdown 递归回退。

两种策略输出相同的 `RetrievalChunk + source_spans`，RAG 和 SectionContext 不感知输入最初是不是 PDF。
...
RAG 与 SectionContext 不需要第二套消费协议。
