...
让 Agent 像阅读代码仓库一样阅读私有学习资料：先由现有 RAG
定位原文，再沿跨文档概念、依赖、引用和来源关系跳转；每次落到另一篇文档后，用该文档的标题树恢复当前位置、父级语境、前置章节和子章节入口。

```text
RAG 命中 RetrievalChunk
  -> 绑定当前 SectionNode，构造局部结构上下文
  -> 绑定 chunk 中有证据的 SemanticAnchor
  -> 跨文档有界扩展
  -> 远端 SourceRef
...
标题树是 Resource 内的结构投影，存 Mongo；Neo4j 只保存跨文档语义锚点和有证据的关系。

本文的 `SemanticAnchor` 是方案层统称；MVP 图模型仍使用现有文档定义的 `Concept`、`Resource` 和
`ExternalSource`，不新增一套平行节点类型。
...
| 分块模型            | 已有 `AUTO/BY_PAGE/BY_TITLE`；通用 parent-child 已删除                 | RAG 在 `application/rag` 投影 retrieval leaf     |
...
| 内容扩展            | `ToolContentWindowBuilder` 按相邻 chunk 序号扩展                      | 根据命中 section 返回祖先、前置兄弟和子章节入口                  |
| 跨文档导航           | 原图增强只在单一 Resource 内找共享实体 chunk                                 | 建立跨 Resource 的语义锚点、关系和 `SourceRef`            |
...
短 Section 可以与相邻同级 Section 装入同一个 retrieval chunk，但每个 Section 的 span 和身份必须保留。长 Section 可以对应多个
retrieval chunks，但始终能从 Kafka 正文范围物化为完整 `SectionView`。`raw_text` 用于证据，带标题路径的 `index_text` 用于
ES/Qdrant。
...
从现有 `TextBlock` 构建版本化 `SectionNode`：记录标题层级、父节点、同级顺序、直接正文范围和子树范围。Retrieval chunks 通过
source spans 与一个或多个 Section 建立映射，不把 chunk 当成 Section 本体。

`locate` 命中和每次远端跳转都生成同一种 `SectionContext`：

- 当前标题路径与命中原文；
- 各级祖先标题及其直接导语；
- 当前节点的前一个同级章节；
- 直接子章节标题和可选短 preview；
- 所有内容的原始 offset、page 和 `content_ref`。

具体结构、分块改动和核心代码见 [title tree](./knowledge_navigate_title_tree.md)。

### 3. 语义锚点完成跨文档跳转

抽取结果先保留 `AnchorMention`，每个 mention 必须指向当前 retrieval leaf 的连续原文 span。canonical anchor
只表达跨文档同一概念，当前语境中的角色说明保留在 mention 或 relation evidence 上。

```text
Chunk A --mention/evidence--> Anchor X <--mention/evidence-- Chunk B
Anchor X --REQUIRES/evidence--> Anchor Y
Resource A --CITES/evidence--> Resource B / ExternalSource
```

共享 anchor 已能产生跨文档候选；`REQUIRES`、`DERIVED_FROM` 等关系用于表达明确的学习路径。抽取 SDK、schema 和 Neo4j
查询见 [graph implementation](./knowledge_navigate_graph_implementation.md)。

### 4. 检索和回读复用现有设施

- ES/Qdrant + `RankingPipeline` 负责入口召回，不再实现一套传统 RAG。
...
5. 迁移 locator、`ToolContentStore`、ES/Qdrant payload 到 source spans。
...
3. `SectionContextBuilder` 生成有预算的局部上下文，写入 `ToolContentStore`。
4. 同 Section 多个 leaf 命中时提升为 SectionView；过长时保留命中片段和继续读取入口。
5. `locate` 返回 source 与 section context；此阶段尚不依赖 Neo4j。

### P3：跨文档共享锚点

1. 对所有有效 changed retrieval leaf 抽取受控类型的 mention，并绑定精确 evidence span。
2. 用 alias exact match、候选向量召回和证据判定解析 canonical anchor。
3. 建立 `chunk -> anchor <- chunk` 的跨 Resource 候选查询。
4. `expand` 跳到远端 SourceRef 后调用同一个 `SectionContextBuilder`。

### P4：显式学习关系

1. 使用固定 schema 抽取依赖、来源、扩展、对比和引用关系。
2. 通过 `EvidenceValidator` 后按 Resource revision 写入 Neo4j。
3. 实现 direction、relation type、depth、ACL、visited set 和整项预算。
4. 关系候选先排序再扩下一跳，避免 hub 广度爆炸。
...
| 跨文档同概念   | 共享 anchor 能找到另一 Resource 的证据   |
| 跨文档依赖/来源 | relation hop 后能在远端标题树中读到完整局部语境 |
...
- 短 Section 是否完整保留 path/span，并始终有 retrieval leaf 进入 ES/Qdrant。
...
- canonical anchor 的过度合并和拆分率是否可接受。
- 共享高频概念是否造成候选爆炸；seed ranking、relation filter 和 depth 2 是否足够。
- LLM 抽取失败是否与“没有关系”严格区分，所有返回边能否回到连续原文。
- Section/Chunk/ES/Qdrant 是否只读取同一个 applied content revision；关系边的 source content revision 是否与它一致。
...
| [relations](./knowledge_navigate_relations.md)                       | 语义锚点、关系、证据校验和多跳阅读                      |
| [graph implementation](./knowledge_navigate_graph_implementation.md) | 抽取 SDK、公开 API、Neo4j 写入和有界检索            |
| [tool contract](./knowledge_navigate_tool_contract.md)               | `locate/expand` 参数、返回值和错误语义            |
